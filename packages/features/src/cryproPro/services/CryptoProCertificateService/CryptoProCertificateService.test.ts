...
import {
  ...
  FormatedCertificate,
} from './CryptoProCertificateService';
...
describe('CryptoProCertificateService.getCerts', () => {
  ...
  it('Props:Skid: происходит поиск сертификата по ключу', () => {
    const certificateBySkid = cryptoProCertificateService.getCertificateBySkid(
  ...
  it('Props:certificates: происходит форматирование списка сертификатов', () => {
    const certificates: FormatedCertificate[] = [
      {
        type: 'ИП',
    ...
    const formatedCertificates =
      cryptoProCertificateService.formatCertificateList(certificatesList);

    expect(formatedCertificates).toEqual(certificates);
  });
});
