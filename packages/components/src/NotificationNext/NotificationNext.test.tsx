...
describe('NotificationNext', () => {
  describe('Progress notify', () => {
    ...
    it('Можно запустить только одно уведомление из одного инстанса', async () => {
      const TestComponent = () => {
        const progressNotify = notify.initProgress({
          containerId: 'progress-notify',
        });
    ...
    it('Можно запустить несколько уведомлений из нескольких инстансов', async () => {
      const TestComponent = () => {
        const progressNotifyOne = notify.initProgress({
          containerId: 'progress-notify',
        });

        const progressNotifyTwo = notify.initProgress({
          containerId: 'progress-notify',
        });

        const handleProgressNotifyOne = () => {
          progressNotifyOne.start('Title');
        };

        const handleProgressNotifyTwo = () => {
          progressNotifyTwo.start('Title');
        };

        return (
          <>
            <NotificationContainerNext containerId="progress-notify" />
            <Button onClick={handleProgressNotifyOne}>Notify one</Button>
            <Button onClick={handleProgressNotifyTwo}>Notify two</Button>
          </>
        );
      };
      ...
      const startOneButton = screen.getByText('Notify one');
      const startTwoButton = screen.getByText('Notify two');

      await userEvents.click(startOneButton);
      await userEvents.click(startTwoButton);
