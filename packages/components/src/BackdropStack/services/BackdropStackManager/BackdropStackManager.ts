...
/**
 * айдишка модалки/поппера/тултипа
 */
type PopId = string;
/**
 * айдишка клика по странице,
...
/**
 * тупл отображающий комбинацию причины закрытия,
 * и айдишки клика в рамках которого происходит закрытие
 */
type PopTuple = [CloseEventReason | undefined, PointerId];

/**
 * расширяемый набор конфликтующих причин закрытия с кликом по бэкдропу модалки,
...
class BackdropStackManager {
  ...
  /**
   * крайняя айдишка клика по странице
   */
  ...
  /**
   * метод проверки на конфликт причины закрытия с предыдущим
   */
  ...
  /**
   * метод генерации случайной айдишки
   */
  public generateID = () => {
  ...
  /**
   * форс метод для удалении подписанного компонента из очереди,
  ...
  /**
   * метод для запроса на выход из очереди,
